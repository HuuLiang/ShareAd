...
- (void)processResponseObject:(id)responseObject withResponseHandler:(QBURLResponseHandler)responseHandler {
    ...
    if ([responseObject isKindOfClass:[NSDictionary class]]) {
        if ([self.response isKindOfClass:[QBURLResponse class]]) {
            ...
            errCode = urlResp.code;
            status = urlResp.code == 200 ? QBURLResponseSuccess : QBURLResponseFailedByInterface;
            errorMessage = (status == QBURLResponseSuccess) ? nil : [NSString stringWithFormat:@"resultCode: %ld", urlResp.code];
        } else {
